...
- `start_day()` (clickup) — Gate-checked against `clickup-state.json` date (returns `"already"` if same logical day); else shells `claude -p /<commands.start_day_skill>` from project root, returns `"ok"`. Default skill: `start-day` at `.claude/skills/start-day/`.
